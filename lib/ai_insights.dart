...
class AIInsights {
  ...
  static String generatePersonalizedInsights(
    String diseaseName,
    String location,
  ) {
    ...
    insights +=
        'This disease is ${isCommon ? "COMMON" : "RARE"} in your region.\n';
    if (isCommon) {
      insights +=
          '⚠️ ${region['nearby_outbreaks']} nearby farmers reported this recently.\n';
    }
    insights += '\n';
    ...
    insights +=
        'Current conditions favor disease spread (${region['rainfall']} rainfall, ${region['temperature']}).\n';
    insights += '\n';

    // PERSONAL HISTORY
    bool hadBefore = history.any((h) => h['disease'] == diseaseName);
    if (hadBefore) {
      var lastOccurrence = history.firstWhere(
        (h) => h['disease'] == diseaseName,
      );
      insights +=
          '📊 Your History: You detected this ${lastOccurrence['date']}.\n';
      insights +=
          'Severity was ${lastOccurrence['severity']}. This is a RECURRING issue.\n';
      insights +=
          '💡 Recommendation: Consider preventive measures even when healthy.\n';
    } else {
      insights += '📊 Your History: First time detecting this disease.\n';
      insights +=
          '💡 Act quickly to prevent spread to your entire ${userProfile['farmSize']} grove.\n';
    }
    insights += '\n';
    ...
    if (diseaseName == 'Peacock Spot') {
      ...
      insights +=
          '• Your last treatment (${userProfile['lastTreatment']}) may need renewal.\n';
      insights +=
          '• With ${userProfile['farmSize']}, expect to need ~15L of copper solution.\n';
      insights +=
          '• Priority: Remove fallen leaves ASAP to stop spore spread.\n';
      insights +=
          '• Timeline: Start treatment within 48 hours for best results.\n';
    } else if (diseaseName == 'Olive Knot') {
      insights += '🌿 For Your Farm:\n';
      insights +=
          '• Immediate action needed - this spreads through rain/irrigation.\n';
      insights +=
          '• Prune infected branches at least 30cm below visible knots.\n';
      insights +=
          '• Disinfect all tools with 10% bleach solution between cuts.\n';
      insights +=
          '• Monitor entire ${userProfile['farmSize']} grove weekly for new infections.\n';
    }
    insights += '\n';
    ...
    insights +=
        '• Weather forecast: Check for rain in next 7 days before spraying.\n';
  ...
  static String generateHealthyInsights(String location) {
    ...
    insights +=
        '• Watch for ${region['common_diseases'].join(", ")} (common in your area).\n\n';

    insights +=
        '🌟 Great job maintaining your ${userProfile['farmSize']} grove!\n';
